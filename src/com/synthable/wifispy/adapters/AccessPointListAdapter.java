...
public class AccessPointListAdapter extends SimpleCursorAdapter
{
	...
	public AccessPointListAdapter(Context context, int layout, Cursor c, String[] from, int[] to) {
	...
	public Object getItem(int position) {
		...
		return mCursor.getInt(mCursor.getColumnIndex(AccessPointAdapter.KEY_ID));
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
    	...
    	mCursor.moveToPosition(position);
		int dbm = mCursor.getInt(mCursor.getColumnIndex(AccessPointAdapter.KEY_DBM));
		ImageView image = (ImageView)row.findViewById(R.id.signalStrengthImage);
		...
		t.setText(
			String.valueOf((mCursor.getInt(mCursor.getColumnIndex(AccessPointAdapter.KEY_ID))))
		);

		TextView c = (TextView)row.findViewById(R.id.channel);
		int channel = WifiSpyService.getChannel(mCursor.getInt(mCursor.getColumnIndex(AccessPointAdapter.KEY_FREQUENCY)));
		c.setText("Ch. "+ channel);

		TextView ssid = (TextView)row.findViewById(R.id.ssid);
		ssid.setText(mCursor.getString(mCursor.getColumnIndex(AccessPointAdapter.KEY_SSID)));

		TextView encryption = (TextView)row.findViewById(R.id.encryption);
		encryption.setText(
			AccessPointAdapter.getEncryptionMethods(
				mCursor.getString( mCursor.getColumnIndex(AccessPointAdapter.KEY_CAPABILITIES) )
			)
		);
